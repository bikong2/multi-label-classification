...
def MobileNet(input_shape=None,
    ...
    x = _conv_bn(img_input, 32, alpha, strides=(2, 2))
...
def _conv_bn(**conv_params):
    ...
    :return: 一个包含卷积和BN层的结构函数，输入张量(rows, cols, in)， 输出张量(rows', cols', out)
    """
    filters = conv_params['filters']
    alpha = conv_params.setdefault('alpha', 1.0)
    kernel_size = conv_params['kernel_size']
    strides = conv_params.setdefault('strides', (1, 1))
    padding = conv_params.setdefault('padding', 'same')
    kernel_initializer = conv_params.setdefault('kernel_initializer', 'he_normal')
    kernel_regularizer = conv_params.setdefault('kernel_regularizer', keras.regularizers.l2(MobileNet.L2_WEIGHT))
    ...
    def f(input_x):
        ...
        bn = keras.layers.BatchNormalization(axis=MobileNet.CHANNEL_AXIS, momentum=MobileNet.BATCH_NORM_DECAY,
                                             epsilon=MobileNet.BATCH_NORM_EPSILON)(conv)
        bn = keras.layers.ReLU(max_value=6.)(bn)
        return bn
