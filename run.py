...
def test_model(classifier):
    ...
    # 实际车牌标签，预测车牌标签，预测概率（label数，验证样本数）
    ...
    valid_mask = (truth != -1)  # 有效的待预测位置标记（排除掉7位车牌的第8位预测结果）
    wrong_mask = abs(pred - truth) > 0.5  # 预测错误的位置标记
    great_conf_mask = (prob >= base_conf)  # 预测置信度大于基线的位置标记
    wrong_plate = np.any(valid_mask & wrong_mask, axis=0)
    great_conf_plate = np.all(~valid_mask | great_conf_mask, axis=0)

    # 总错误数，大于置信度错误数，总大于置信度样本数
    wrong_count = np.sum(wrong_plate)
    great_total_count = np.sum(great_conf_plate)
    great_wrong_count = np.sum(wrong_plate & great_conf_plate)
    # 记录大于置信度的预测错误车牌
    if np.any(wrong_plate & great_conf_plate):
        great_wrong_records = [u.decode() for u in great_wrong_records[wrong_plate & great_conf_plate]]
...
def plot_confusion_matrix(y_trues, y_preds):
    ...
    for i in range(y_trues.shape[0]):
        ...
        draw_tools.plot_confusion_matrix(y_trues[i][valid_mask], y_preds[i][valid_mask],
                                         plate.get_chars_sorted_by_label(i),
